...
- `skills/.experimental/<skill-name>/SKILL.md`: entrypoint and workflow for a single skill (required).
- `skills/.experimental/<skill-name>/references/`: optional docs/links the skill relies on.
- `skills/.experimental/<skill-name>/scripts/`: optional helper scripts used by the skill.
- `skills/.experimental/<skill-name>/assets/`: optional templates, examples, or UI assets.

When a skill is stable, promote it to `skills/.curated/<skill-name>/`.
...
- Tests live in `skills/.experimental/<skill-name>/tests/` (or `skills/.curated/<skill-name>/tests/`) or a top-level `tests/`.
